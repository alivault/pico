...
struct GitWorkspaceView: View {
  ...
  init(
    ...
    initiallyShowsFiles: Bool = false
  ) {
    self.model = model
    self.directory = directory
    _selectedTab = State(initialValue: initiallyShowsFiles ? .allFiles : .changes)
  }
  ...
  private var content: some View {
    if cwd == nil {
      GitEmptyView(
        ...
        systemImage: "folder",
        message:
          "Select or create a session with a project directory to browse files and Git changes."
      )
...
private struct GitWorkspaceData: Equatable {
  ...
  var commitEntries: [GitCommitGraphEntry] {
    commits.map(GitFormatting.parseCommitGraphLine).filter {
      !$0.hash.isEmpty || !$0.subject.isEmpty
    }
  }
}
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 10) {
      HStack(alignment: .center, spacing: 10) {
        ...
        .picoGlassButtonStyle(.prominent)
