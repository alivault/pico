...
struct ConversationScreen: View {
  ...
  private func showHeaderCommitSheet() {
    guard !isPreparingHeaderCommit,
      let cwd = model.conversationGitDirectory
    else {
      return
    }
...
private struct EditUserMessageSheet: View {
  ...
  private var submitDisabled: Bool {
    isSubmitting || isResending
      || (text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty && item.images.isEmpty)
  }
...
#if os(macOS)
  private struct ConversationHeaderVisibilityToggles: View {
    @Bindable var model: AppModel

    var body: some View {
      Toggle(isOn: thinkingIsVisible) {
        Label(
          thinkingIsVisible.wrappedValue ? "Hide Thinking" : "Show Thinking",
          picoSystemImage: "brain",
          size: 18
        )
      }
      .toggleStyle(.button)
      ...
      .labelStyle(.iconOnly)
      .help(thinkingIsVisible.wrappedValue ? "Hide Thinking" : "Show Thinking")

      Toggle(isOn: toolsAreVisible) {
        Label(
          toolsAreVisible.wrappedValue ? "Hide Tools" : "Show Tools",
          picoSystemImage: "wrench.and.screwdriver",
          size: 18
        )
      }
      .toggleStyle(.button)
      .tint(Color.accentColor)
      .labelStyle(.iconOnly)
      .help(toolsAreVisible.wrappedValue ? "Hide Tools" : "Show Tools")
    }

    private var thinkingIsVisible: Binding<Bool> {
      Binding(
        get: { !model.sessionState.hideThinkingBlock },
        set: { isVisible in
          Task {
            await model.setThinkingHidden(!isVisible)
          }
        }
      )
    }

    private var toolsAreVisible: Binding<Bool> {
      Binding(
        get: { !model.hideToolBlocks },
        set: { isVisible in
          model.setToolBlocksHidden(!isVisible)
        }
      )
    }
  }
