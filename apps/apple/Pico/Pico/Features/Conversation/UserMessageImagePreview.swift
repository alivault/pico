...
struct UserMessageImagePreview: View {
  ...
  private var previewContent: some View {
    if images.isEmpty {
      ...
      #else
        ZStack {
          ZoomableUserMessageImagePage(image: images[selectedIndex]) { isZoomed in
            setImageZoomed(isZoomed, at: selectedIndex)
          }
          .accessibilityLabel(imageAccessibilityLabel(at: selectedIndex))

          VStack {
            HStack {
              Spacer()
              Button("Close", picoSystemImage: "xmark", action: dismiss.callAsFunction)
                .labelStyle(.iconOnly)
            ...
            if images.count > 1 {
              HStack {
                Button("Previous Image", picoSystemImage: "chevron.left") {
                ...
                Button("Next Image", picoSystemImage: "chevron.right") {
                  selectedIndex = min(images.count - 1, selectedIndex + 1)
                }
  ...
  private var accessibilityHint: String {
    if images.count > 1 {
      return
        "Swipe horizontally to view other attachments. Swipe down to close. Double tap an image to zoom."
    }
