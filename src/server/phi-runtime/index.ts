...
} from "@/server/phi-runtime/contexts"
import {
  buildHighlightPayload,
  type HighlightPayload,
} from "@/server/phi-runtime/highlight"
import {
  applyRetainedConversationEvent,
  createRetainedConversationState,
} from "@/server/phi-runtime/conversation-retainer"
import {
...
export class PhiRuntime {
  ...
  private logSessionLoadDebug(
    ...
    console.log(`[phi:session-load] ${event}${suffix}`)
  }
      ...
      const prefix =
        diagnostic.type === "error"
          ...
          : diagnostic.type === "warning"
            ? "warn"
            : "info"
      console.log(`[phi:${prefix}] ${diagnostic.message}`)
    }
  }
  ...
  private async readSessionIndexEntries() {
    ...
    try {
      ...
      console.error("[phi] failed to list sessions:", error)
      return []
    }
  }
  ...
  private async disposeSessionEntry(entry: SessionEntry) {
    ...
    try {
      ...
      console.error("[phi] session dispose error:", error)
    }
  ...
  private emitAutoSessionNamingFailure(
    ...
    console.error("[phi] auto session naming failed:", {
      sessionId: entry.session.sessionId,
      cwd: entry.cwd,
    ...
    await session.bindExtensions({
      ...
      shutdownHandler: () => {
        this.dispose().catch((error) => {
          console.error("[phi] shutdown failed:", error)
        })
      },
  ...
  async highlightCode(code: unknown, language: unknown) {
    ...
    try {
      ...
      if (!this.highlightLoadErrorLogged) {
        ...
        console.warn(
          `[phi:warn] Syntax highlighting unavailable: ${formatError(error)}`
        )
      }
      return {
        ok: true,
...
declare global {
  ...
  var __phiRuntime: PhiRuntime | undefined
}

export function getPhiRuntime() {
  globalThis.__phiRuntime ??= new PhiRuntime()
  return globalThis.__phiRuntime
}
