...
const SIDEBAR_WIDTH_MIN = "16rem"
const SIDEBAR_WIDTH_MAX = "32rem"
const SIDEBAR_WIDTH_STORAGE_KEY = "pico-sidebar-width"
const SIDEBAR_WIDTH_LEGACY_STORAGE_KEY = "sidebar_width"
...
function normalizeSidebarWidth(value: string | null | undefined) {
  const defaultWidth = parseCssLengthToPixels(SIDEBAR_WIDTH, 320)
  const minWidth = parseCssLengthToPixels(SIDEBAR_WIDTH_MIN, 256)
  const maxWidth = parseCssLengthToPixels(SIDEBAR_WIDTH_MAX, 512)
  const width = parseCssLengthToPixels(value ?? "", defaultWidth)

  return `${Math.round(Math.min(Math.max(width, minWidth), maxWidth))}px`
}

function readStoredSidebarWidth() {
  if (typeof window === "undefined") return normalizeSidebarWidth(SIDEBAR_WIDTH)

  try {
    return normalizeSidebarWidth(
      window.localStorage.getItem(SIDEBAR_WIDTH_STORAGE_KEY) ??
        window.localStorage.getItem(SIDEBAR_WIDTH_LEGACY_STORAGE_KEY)
    )
  } catch {
    return normalizeSidebarWidth(SIDEBAR_WIDTH)
  }
}
...
function SidebarProvider({
  ...
  React.useEffect(() => {
    const nextWidth = readStoredSidebarWidth()
      ...
      <div
        ...
        className={cn(
          ...
          variant === "floating" || variant === "inset"
            ...
            : "border-sidebar-border/70 group-data-[collapsible=icon]:w-(--sidebar-width-icon) group-data-[side=left]:border-r group-data-[side=right]:border-l",
          className
        )}
        {...props}
      >
...
function SidebarTrigger({
  ...
  return (
    <TitleTooltip title="Toggle sidebar" kbd="⌃ B">
      <Button
...
function SidebarRail({
  ...
  const { handleClick, handlePointerDown } = useSidebarResize({
    isMobile,
...
function SidebarGroupAction({
  ...
  return useRender({
    ...
    props: mergeProps<"button">(
      {
        className: cn(
          "absolute top-3.5 right-3 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-sidebar-foreground ring-sidebar-ring outline-hidden transition-transform group-data-[collapsible=icon]:hidden after:absolute after:-inset-2 hover:bg-sidebar-accent/50 hover:text-sidebar-accent-foreground focus-visible:ring-2 md:after:hidden [&>svg]:size-4 [&>svg]:shrink-0",
          className
        ),
      },
      props
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm ring-sidebar-ring outline-hidden transition-[width,height,padding] group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! hover:text-sidebar-accent-foreground not-data-active:hover:bg-sidebar-accent/50 focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:hover:bg-sidebar-accent/50 data-open:hover:text-sidebar-accent-foreground data-active:bg-sidebar-accent data-active:text-sidebar-accent-foreground [&_svg]:shrink-0 [&>span:last-child]:truncate [&>svg]:size-4",
  {
    variants: {
      variant: {
        default:
          "hover:text-sidebar-accent-foreground not-data-active:hover:bg-sidebar-accent/50",
        outline:
          "bg-background shadow-[0_0_0_1px_hsl(var(--sidebar-border))] hover:text-sidebar-accent-foreground hover:shadow-[0_0_0_1px_hsl(var(--sidebar-accent))] not-data-active:hover:bg-sidebar-accent/50",
      },
      size: {
        default: "h-8 text-sm",
        sm: "h-7 text-xs",
