...
import type { ForkMessage } from "@/features/phi/app-shell-dialog-types"
import type {
  ...
  const deleteCleanup = async () => {
    try {
      ...
      await queryClient.invalidateQueries({
        queryKey: phiQueryKeys.directorySessionsIndex(
          viewerContextId,
          directory
        ),
      })
...
export function ForkSessionDialogController({
  ...
  const openRef = React.useRef(open)
  const queryClient = useQueryClient()
  const queryKey = phiQueryKeys.forkableMessages(
    viewerContextId,
    sessionScopeKey
  )
